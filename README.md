...
- **購買紀錄**：每筆持倉可記錄多筆買入 / 賣出交易，自動計算加權成本與未實現損益（%）
- **類別分組**：持倉與交易紀錄依類別分組顯示，支援展開 / 收合
...
在「購買紀錄」分頁，點每筆倉位右側的「**+ 新增**」，輸入：
- 類型（買入 / 賣出）、日期、數量、單價、幣別

系統自動依此計算**加權平均成本**與**未實現損益**。
...
- 倉位明細：點類別標題列展開/收合該類別的倉位
- 購買紀錄：點倉位標題列展開/收合交易明細
