...
import { AMOUNT_MASK } from '@/lib/constants';
...
export function LotTable({ positions, usdToTwd, preferredCurrency = 'USD', hideAmounts = false, onAdd, onEdit, onDelete }: LotTableProps) {
  const eligible = positions.filter((p) => p.market !== 'cash');
  const hasAnyLots = eligible.some((p) => (p.lots?.length ?? 0) > 0);
